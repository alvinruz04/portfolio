...
/** REQUIRED FIELDS */
if ($seller_code === '') {
    json_fail('Seller code is required.', 422);
}

if ($full_name === '') {
    json_fail('Full name is required.', 422);
}

if ($position === '') {
    json_fail('Position is required.', 422);
}

if ($area_of_distribution === '') {
    json_fail('Area of distribution is required.', 422);
}

if ($contact_number === '') {
    json_fail('Contact number is required.', 422);
}
...
/** UNIQUE seller_code */
$chk = $conn->prepare("SELECT id FROM sellers WHERE seller_code = ? LIMIT 1");
$chk->bind_param("s", $seller_code);
$chk->execute();
...
if (isset($_FILES['avatar']) && ($_FILES['avatar']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) {
    try {
        $avatar = save_seller_avatar($_FILES['avatar']);
    } catch (Throwable $e) {
...
if (!$stmt->execute()) {
    ...
    json_fail('Failed to create seller: ' . $stmt->error, 500);
}
