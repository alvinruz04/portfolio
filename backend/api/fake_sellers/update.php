...
$get = $conn->prepare("SELECT * FROM fake_sellers WHERE id = ? LIMIT 1");
$get->bind_param("i", $id);
$get->execute();
...
$chk = $conn->prepare("
    SELECT id
    FROM fake_sellers
    WHERE shop_name = ? AND shop_link = ? AND id <> ?
    LIMIT 1
");
$chk->bind_param("ssi", $shop_name, $shop_link, $id);
$chk->execute();
...
if (!$stmt->execute()) {
    json_fail('Failed to update fake seller: ' . $stmt->error, 500);
}
$stmt->close();
...
audit_write($conn, [
    ...
    'action'      => 'FAKE_SELLER_UPDATE',
    'entity_type' => 'fake_seller',
    'entity_id'   => $id,
    'meta' => [
        'message' => 'Fake seller updated',
        'old_shop_name' => (string)($existing['shop_name'] ?? ''),
        'new_shop_name' => $shop_name,
        'old_shop_link' => (string)($existing['shop_link'] ?? ''),
        'new_shop_link' => $shop_link,
        'platform' => $platform,
